import { HookClient, InjectContext, RpgCommonPlayer, RpgPlugin, Utils } from "@rpgjs/common";
...
export class RpgClientEngine {
  ...
  constructor(private context: Context) {
    ...
    this.gameEngine = inject(context, GameEngineClient);
    this.renderer = inject(context, RpgRenderer);
  }
  ...
  async connection(uri?: string) {
    ...
    this.world.listen(this.socket).value.subscribe(
      async (val: {
        data: any;
        partial: any;
        time: number;
